...
1. **대화 상대 선택** (대화 시작 전에만 가능):
   - **제3의 인물 (처음 보는 낯선 사람)**: 캐릭터가 처음 보는 완전한 낯선 사람으로 인식
   - **다른 주인공 (책 속 인물)**: 같은 책의 다른 주인공으로 인식 (예: Victor Frankenstein 선택 시 "The Creature"로 표시)
   - ⚠️ **주의**: 대화가 시작되면 선택을 변경할 수 없습니다
2. **첫 메시지 입력**: 시나리오에 맞는 질문이나 인사
3. **대화 진행**: 최대 5턴까지 대화 가능
4. **대화 저장/취소**: 5턴 완료 후 저장 또는 취소 선택
...
2. **데이터 파일**: `data/characters/` 폴더에 책별 캐릭터 JSON 파일이 있어야 합니다 (또는 레거시 `data/characters.json`).
...
### 캐릭터를 찾을 수 없음
- `data/characters/` 폴더에 책별 JSON 파일이 있는지 확인 (새 구조)
- 또는 `data/characters.json` 파일이 있는지 확인 (레거시)
- 캐릭터 이름이 정확한지 확인
